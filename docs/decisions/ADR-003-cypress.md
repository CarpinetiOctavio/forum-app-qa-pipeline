...
- **The `cypress-e2e` CI job runs a real backend, not just the frontend, because the non-mocked `DeleteComment` spec needs one.** The mocked specs never touch it (`cy.intercept()` answers every request before it reaches the network), but they run in the same job rather than a second one, since starting the backend costs nothing to specs that ignore it. The backend is built and started in its own step (`go build` + backgrounded `./app`), health-checked with `wait-on http://localhost:8080/api/posts` before the frontend/Cypress step starts — this repo has no dedicated `/health` endpoint, and adding one solely for a CI wait condition wasn't judged worth the extra surface; an existing, always-available real route serves the same purpose. Kept as a separate step from `cypress-io/github-action`'s own `wait-on` (which only covers the frontend) so a failure names the specific process that didn't come up, rather than surfacing as a generic Cypress failure.
