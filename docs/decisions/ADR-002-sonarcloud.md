...
- **SonarGo doesn't auto-recognize `_test.go` as test code the way SonarJS auto-recognizes `.test.tsx`.** First discovered when `main`'s first real branch analysis (after the `push`-trigger fix) reported `backend/tests/services/post_service_test.go` at 0% new coverage with 143 uncovered lines — the test file itself, not application code. Confirmed against SonarSource's own Go documentation: without `sonar.tests`/`sonar.test.inclusions` set, every line in a `_test.go` file counts as needing coverage and shows 0%, since `coverage.out` never instruments a test file's own lines, only what it exercises.
  ...
  **A third instance of the same bug, this time on the frontend side, surfaced immediately after the `sonar.sources` fix made real analysis of `frontend/` possible for the first time.** `main`'s first full-scope branch analysis failed its Quality Gate on Coverage on New Code (24.59%, required ≥80%). Checked file-by-file via SonarCloud's `measures/component` API rather than guessing from the aggregate number: `frontend/cypress/e2e/posts.cy.ts` showed `new_lines_to_cover: 98` at `new_coverage: 0.0%`; `PostDetail.test.tsx`, `CommentList.test.tsx`, and `postService.test.ts` (all touched 2026-07-30, within the New Code window) showed the identical pattern (68, 41, and 22 uncovered new lines respectively). This directly contradicts this ADR's own earlier claim that "SonarJS auto-recognizes `.test.tsx`" — a claim that had never actually been checked against a real analysis of this repo, since no frontend file had ever been indexed before the `sonar.sources` fix. One frontend test file, `PostList.test.tsx`, did show `new_lines_to_cover: 0` — but not because it was correctly classified as a test: it was last modified 2026-07-27, older than the New Code window, so none of its lines counted as "new" regardless of classification. That made it a false negative in the initial check, not a counterexample; ruled out by comparing each file's last-modified date against the New Code period boundary before concluding anything.
  Fixed by widening `sonar.test.inclusions` only — `backend/tests/**,frontend/src/**/*.test.ts,frontend/src/**/*.test.tsx,frontend/cypress/e2e/**/*.cy.ts` — without touching `sonar.tests`, which stays `backend/tests`. Widening `sonar.tests` itself to a frontend root (e.g. `frontend/src`) would have reproduced the original `backend/`-scope bug one level down: `frontend/src` mixes real components/services with their `.test.tsx` files in the same directories, unlike `backend/tests`, which holds nothing but tests and mocks — marking all of `frontend/src` as a tests root would misclassify `PostDetail.tsx`, `postService.ts`, and every other production file alongside it as tests too, dropping them out of coverage measurement entirely. `sonar.test.inclusions` is a pattern match, not a root directory, so it can select just the test-shaped files inside a mixed directory without that risk.
  To verify once this fix lands: the same per-file `measures/component` check in both directions — `PostDetail.test.tsx`/`posts.cy.ts` should drop to `new_lines_to_cover: 0` (fixed), while `PostDetail.tsx`/`postService.ts` (real frontend source, not test) must still show up as indexed with real `new_lines_to_cover` counts — if those disappear or go to 0 too, the fix reproduced the backend bug in frontend and needs to be reverted. Pending verification against the next real CI run before this is merged.
